...
exit 1 unless
   GetOptions("dump=s" => \$opts{dumptype},
             ...
             "cache-dir=s" => \$opts{cache_dir});

$opts{user} = 'ccpro';
$opts{password} = 'whi1t3r';
$opts{journal} = 'ccpro.livejournal.com';
$opts{sync} = 1;
...
jbackup.pl -- journal database generator and formatter
  ...
  Data update options:
   ...
   --cache-dir=X   Specify the directory in which to store cache files.
...
my %bak;
my $filename = "$opts{cache_dir}/$opts{user}." . ($opts{usejournal} ? "$opts{usejournal}." : '') . "jbak";
...
warn Dumper($meta{$curid}{$lasttag}, $_[1]);
...
print '2'.Dumper($content);
   ...
   foreach my $id (keys %meta) {
       ...
       $count++;
print "save_comment id: $id, jitemid: $meta{$id}{jitemid}, ".Dumper($meta{$id})."\n";
       save_comment($meta{$id}{jitemid}, $meta{$id});
   }
...
sub save_event($)
{
   ...
   $item_id = $data->{ditemid};

#print Dumper($data->{props});
#   if ($data->{url} =~ /^#(.*)\.html$/) {
#     $item_id = int $1;
#   }
#   else {
...
#   }
   ...
   my ($count, $revnum) = $dbi->selectrow_array($sql = "select count(*), revision from live_journal where itemid = $item_id group by revision");

   $revnum = int $revnum;
#print 'md5_hex from db '.md5_hex($event)."\n";
#print 'md5_hex from lj '.md5_hex($data->{event})."\n";
...
#<>;
 ...
 my $insert = 0;
   ...
   if ($count == 0) {
      ...
      if (!exists($data->{props}{revnum})) {
         $update = 1;
      }
      elsif (int $data->{props}{revnum} != $revnum) {
         $update = 1;
      }
   }
   ...
   if ($update) {
     ...
     $sql = "UPDATE live_journal
             SET current_mood  = ".$dbi->quote($data->{props}{current_mood}).",
                current_music = ".$dbi->quote($data->{props}{current_music}).",
                subject      = ".$dbi->quote($data->{subject}).",
                allow_mask   = ".($data->{allow_mask}||0).",
                event        = ".$dbi->quote($data->{event}).",
                eventtime    = ".$dbi->quote($data->{eventtime}).",
                tags         = ".$dbi->quote($data->{props}{taglist}||'').",
                logtime      = ".$dbi->quote($data->{eventtime}).",
                location     = ".$dbi->quote($data->{props}{current_location}).",
                revision     = ".($data->{props}{revnum}||0).",
                security     = ".$dbi->quote($data->{security})."
       ...
       $sql = "INSERT INTO live_journal (
               ...
               allow_mask, event, eventtime, tags, logtime, location, revision, security)
             VALUES(".$item_id.",".$dbi->quote($data->{props}{current_mood}).",".
                    $dbi->quote($data->{props}{current_music}).",".
                    $dbi->quote($data->{subject}).",".
                    ($data->{allow_mask}||0).",".
                    $dbi->quote($data->{event}).",".
                    $dbi->quote($data->{eventtime}).",".
                    $dbi->quote($data->{props}{taglist}||'').",".
                    $dbi->quote($data->{eventtime}).",".
                    $dbi->quote($data->{props}{current_location}).",".
                    ($data->{props}{revnum}||0).",".
                    $dbi->quote($data->{security}).")";
     }

#print 'mood :'.length($data->{props}{current_mood})."\n";
#print 'music :'.length($data->{props}{current_music})."\n";
#print "$sql\n";
     $dbi->do($sql);
...
sub dump_xml($$$)
{
   ...
   $cdumper = sub {
       ...
       foreach my $data (@{$ary || []}) {
          ...
          $res .= ">\n";
	    #
	    # Remove 'T' and 'Z' from timestamp
	    my $fixed_date = $data->{date};
	    $fixed_date =~ s/T/\ /;
	    $fixed_date =~ s/Z//;
          #$res .= "\t\t\t\t<date>$data->{date}</date>\n";
          $res .= "\t\t\t\t<date>$fixed_date</date>\n";
   ...
   foreach my $evt (sort { $a->{eventtime} cmp $b->{eventtime} } values %{$events || {}}) {
       my $ditemid = $evt->{itemid} * 256 + $evt->{anum};
       $ret .= "\t<entry jitemid='$evt->{itemid}' anum='$evt->{anum}' ditemid='$ditemid'";
...
sub exml($)
{
 my ($e) = @_;
   ...
   return $e unless $e =~ /[&\"\'<>\x00-\x08\x0B\x0C\x0E-\x1F]/;
